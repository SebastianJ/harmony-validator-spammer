...
upload-path-linux := 's3://tools.harmony.one/release/linux-x86_64/harmony-validator-spammer'

dist := ./dist/harmony-validator-spammer
env := GO111MODULE=on
DIR := ${CURDIR}
...
upload-linux:static
	aws s3 cp dist/harmony-validator-spammer ${upload-path-linux} --acl public-read
